...
public class MessageController {
    ...
    private static final int MAGIC_ROOM = 1408;
    ...
    public void getMessage(@DestinationVariable String roomId, Message message) {
        if (parseRoomId(roomId) != MAGIC_ROOM) {
            ...
            template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, MAGIC_ROOM),
                    new Message(HtmlUtils.htmlEscape(message.messageStr())));
        }
    }
    ...
    public void handleSessionSubscribeEvent(SessionSubscribeEvent event) {
        ...
        Flux<Message> messageFlux = roomId == MAGIC_ROOM ? getAllMessages() : getMessagesByRoomId(roomId);
        messageFlux
